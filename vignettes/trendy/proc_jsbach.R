...
nc <- nc_open("~/../../data_2/scratch/ttrinidad/data/trendy/raw/JSBACH_S3_gpp.nc")
...
gpp <- array(gpp, dim = c(192, 96, 12, 323))
...
gpp_month <- gpp*(array(seconds_matrix, dim = c(192,96,12,323)))
...
pseudo_raster <- raster::raster(nrows = 96, ncols = 192, xmn = -180, xmx = 178.125, ymn = -88.57217, ymx = 88.57217)
area_raster <- (raster::area(pseudo_raster))*1e6 # returning filed areas, connverted from km2 to m2
area_matrix <- t(matrix(raster::getValues(area_raster), nrow= 96, ncol = 192)) # Need to transpose here as lon, lat are flipped in the pseudo_raster
...
jsbach_gpp_annual_global <- gpp_annual_global*1e-12 #in Pg*C

# IAV of global annual GPP
jsbach_gpp_iav <- sd(jsbach_gpp_annual_global)
...
nc <- nc_open("~/../../data_2/scratch/ttrinidad/data/trendy/raw/JSBACH_S3_nbp.nc")
...
# temporal aggregation:
## separate months and years axes
nbp <- array(nbp, dim = c(192, 96, 12, 323))

## multiply weightening time matrix with nbp values
nbp_month <- nbp*(array(seconds_matrix, dim = c(192,96,12,323)))
...
jsbach_nbp_annual_global <- nbp_annual_global*1e-12 #in Pg*C


# IAV of global annual NBP
jsbach_nbp_iav <- sd(jsbach_nbp_annual_global)

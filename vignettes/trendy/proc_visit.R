...
nc <- nc_open("~/../../data_2/scratch/ttrinidad/data/trendy/raw/VISIT_S3_gpp.nc")
...
gpp <- array(gpp, dim = c(720, 360, 12, 163))
...
seconds_matrix <- matrix(rep(seconds_months, times = 163), nrow = 12, ncol = 163)

## multiply weightening time matrix with nbp values
gpp_month <- gpp*(array(seconds_matrix, dim = c(720,360,12,163)))
...
visit_gpp_annual_global <- gpp_annual_global*1e-12 #in Pg*C

# IAV of global annual GPP
visit_gpp_iav <- sd(visit_gpp_annual_global)
...
nc <- nc_open("~/../../data_2/scratch/ttrinidad/data/trendy/raw/VISIT_S3_nbp.nc")
...
# temporal aggregation:
## separate months and years axes
nbp <- array(nbp, dim = c(720, 360, 12, 163))

## multiply weightening time matrix with nbp values
nbp_month <- nbp*(array(seconds_matrix, dim = c(720,360,12,163)))
...
visit_nbp_annual_global <- nbp_annual_global*1e-12 #in Pg*C


# IAV of global annual NBP
visit_nbp_iav <- sd(visit_nbp_annual_global)

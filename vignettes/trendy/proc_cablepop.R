...
nc <- nc_open("~/../../data_2/scratch/ttrinidad/data/trendy/raw/CABLE-POP_S3_gpp.nc")
...
cablePop_gpp_annual_global <- gpp_annual_global*1e-12 #in Pg*C

# IAV of global annual GPP
cablePop_gpp_iav <- sd(cablePop_gpp_annual_global)
...
nc <- nc_open("~/../../data_2/scratch/ttrinidad/data/trendy/raw/CABLE-POP_S3_nbp.nc")
...
# temporal aggregation:
## separate months and years axes
nbp <- array(nbp, dim = c(360, 180, 12, 323))

## multiply weightening time matrix with nbp values
nbp_month <- nbp*(array(seconds_matrix, dim = c(360,180,12,323)))
...
cablePop_nbp_annual_global <- nbp_annual_global*1e-12 #in Pg*C


# IAV of global annual NBP
cablePop_nbp_iav <- sd(cablePop_nbp_annual_global)

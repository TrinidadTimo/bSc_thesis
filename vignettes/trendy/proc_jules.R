...
nc <- nc_open("~/../../data_2/scratch/ttrinidad/data/trendy/raw/JULES_S3_gpp.nc")
...
gpp <- array(gpp, dim = c(192, 144, 12, 323))
...
gpp_month <- gpp*(array(seconds_matrix, dim = c(192,144,12,323)))
...
pseudo_raster <- raster::raster(nrows = 144, ncols = 192, xmn = 0.9375, xmx = 359.0625, ymn = -89.375, ymx = 89.375)
area_raster <- (raster::area(pseudo_raster))*1e6 # returning filed areas, connverted from km2 to m2
area_matrix <- t(matrix(raster::getValues(area_raster), nrow= 144, ncol = 192)) # Need to transpose here as lon, lat are flipped in the pseudo_raster
...
jules_gpp_annual_global <- gpp_annual_global*1e-12 #in Pg*C

# IAV of global annual GPP
jules_gpp_iav <- sd(jules_gpp_annual_global)
...
nc <- nc_open("~/../../data_2/scratch/ttrinidad/data/trendy/raw/JULES_S3_nbp.nc")
...
# temporal aggregation:
## separate months and years axes
nbp <- array(nbp, dim = c(192, 144, 12, 323))

## multiply weightening time matrix with nbp values
nbp_month <- nbp*(array(seconds_matrix, dim = c(192,144,12,323)))
...
jules_nbp_annual_global <- nbp_annual_global*1e-12 #in Pg*C


# IAV of global annual NBP
jules_nbp_iav <- sd(jules_nbp_annual_global)

...
nc <- nc_open("~/../../data_2/scratch/ttrinidad/data/trendy/raw/CLASSIC_S3_gpp.nc")
...
gpp <- array(gpp, dim = c(360, 180, 12, 322))
...
seconds_matrix <- matrix(rep(seconds_months, times = 322), nrow = 12, ncol = 322)

## multiply weightening time matrix with nbp values
gpp_month <- gpp*(array(seconds_matrix, dim = c(360,180,12,322)))
...
pseudo_raster <- raster::raster(nrows = 180, ncols = 360, xmn = -180, xmx = 179, ymn = -89.5, ymx = 89.5)
...
classic_gpp_annual_global <- gpp_annual_global*1e-12 #in Pg*C

# IAV of global annual GPP
classic_gpp_iav <- sd(classic_gpp_annual_global)
...
nc <- nc_open("~/../../data_2/scratch/ttrinidad/data/trendy/raw/CLASSIC_S3_nbp.nc")
...
# temporal aggregation:
## separate months and years axes
nbp <- array(nbp, dim = c(360, 180, 12, 322))

## multiply weightening time matrix with nbp values
nbp_month <- nbp*(array(seconds_matrix, dim = c(360,180,12,322)))
...
classic_nbp_annual_global <- nbp_annual_global*1e-12 #in Pg*C


# IAV of global annual NBP
classic_nbp_iav <- sd(classic_nbp_annual_global)
